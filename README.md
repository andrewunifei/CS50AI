...
**Minesweeper Project** (Lecture 01): [pygame](https://github.com/pygame/)

**Shopping Project** (Lecture 04): [scikit-learn](https://github.com/scikit-learn/)

**Traffic Project** (Lecture 05): [tensorflow](https://github.com/tensorflow), [openCV](https://github.com/opencv), [scikit-learn](https://github.com/scikit-learn/)
...
## Lecture 06 - Language
Nesse aula foi abordado o conceito de Processamento de Linguagem Natural (PLN). O PLN expressa algumas tarefas comuns que uma IA pode performar, como por exemplo, resumo automático (*automatic summarization*), extração de informação (*information extraction*), identificação de língua (*language identification*), tradução automática (*machine translation*), reconhecimento de entidade mencionada (*named entity recognition*) reconhecimento de fala (*speech recognition*), classificação de texto (*text classification*) e desambiguação (*word sense disambiguation*). Existem muitas outras tarefas dentro desse campo. A aula abordou apenas os conceitos fundamentais de forma superficial.

Normalmente, ao abordar o conceito de linguagem, leva-se em consideração duas coisas centrais: a **sintaxe** e a **semântica**. A sintaxe se refere a estrutura da linguagem e o funcionamento dessa estrutura. Já a semântica diz respeito a o que uma palavra ou sequência de palavras significa. Existem exemplos de sentenças que sintaticamente são bem estruturadas, mas não expressam nenhum significado. O linguista Noam Chomsky dá o seguinte exemplo: "Ideias verdes sem cores dormem furiosamente".

É necessário um conjunto de regras a fim de demonstrar para a IA quais conjuntos de palavras são sentenças válidas e quais não são. A partir disso, o objetivo é que a IA faça inferências em relação ao significado dessas sentenças. Os métodos apresentados na aula para modelar essas regras foram: *formal grammar* e *context-free grammar*. 

Além disso, como podem haver textos enormes, foi apresentado métodos para dividir um corpo de texto em partes menores a fim de facilitar a análise. Esses métodos são: *n-grams* e *tokenization*.

Também foi abordado a noção de categorização de texto como uma ferramente útil em certos cenários, por exemplo, categorização de e-mails (*inbox* ou *spam*) e, dado um texto, esse texto expressa um sentimento negativo ou positivo? Esse último exemplo pode parecer em avaliações de produtos em sites de vendas. Os métodos de classificação abordados foram: *bag-of-words model* e *naive Bayes* com *Lapace smoothing*.
...
A partir disso, entra-se no campo da semântica. Intuitivamente, uma abordagem para resolver o problema da semântica é reunir as palavras e seus significados em uma conjunto de dados. Um exemplo disso é o banco de dados **WordNet**. O problema dessa ideia é a incompatibilidade com a mudanças da linguagem e suas relações mais complexas. Por conta disso, uma abordagem mais sofisticada seria representar cada palavra como um vetor de diferentes números, esse método é denominado *distribution representation*. Dessa forma, é possível representar palavras que têm significados semelhantes como vetores semelhantes. Assim, o significado das palavras são obtidos considerando quais outras palavras estão próximas delas em determinado contexto. Um modelo para fazer essa transformação palavra-vetor é o **Word2Vec**. Finalmente, foi apresentado a estrutura **skip-gram architecture**, uma rede neural que faz uso da abordagem Word2Vec para prever o contexto de palavras dado uma palavra.

### Projects
O site da *Harvard University* disponibilizou dois projetos que abordam o tema explorado nesse aula, as soluções dos projetos se encontram na pasta *Lecture06 - Language* desse repositório.

O primeiro projeto é uma implementação do método de estruturamento de sentenças denomidado "*context-free grammar*" que toma como parâmetro uma sentença e retorna a estrutura sintática dessa sentença.

O segundo projeto é uma implementação de um sistema de perguntas e respostas. O usuário entra com uma pergunta e a IA decide qual a resposta mais apropriada. Faz uso de duas tarefas: *document retrieval* e *passage retrieval*.
