...
## Lecture 06 - Language
Nesse aula foi abordado o conceito de Processamento de Linguagem Natural (PLN). O PLN expressa algumas tarefas comuns que uma IA pode performar, como por exemplo, resumo automático (*automatic summarization*), extração de informação (*information extraction*), identificação de língua (*language identification*), tradução automática (*machine translation*), reconhecimento de entidade mencionada (*named entity recognition*)m reconhecimento de fala (*speech recognition*), classificação de texto (*text classification*) e desambiguação (*word sense disambiguation*). Existem muitas outras tarefas dentro desse campo. A aula abordou apenas os conceitos fundamentais de forma superficial.

Normalmente, ao abordar o conceito de linguagem, leva-se em consideração duas coisas centrais: a **sintaxe** e a **semântica**. A sintaxe se refere a estrutura da linguagem o funcionamento dessa estrutura. Já a semântica diz respeito a o que uma palavra ou sequência de palavras significa. Existem exemplos de sentenças que sintaticamente são bem estruturadas, mas não expressam nenhum significado. O linguista Noam Chomsky dá o seguinte exemplo: "Ideias verdes sem cores dormem furiosamente".
...
Além disso, como podem haver textos enormes, foi apresentado métodos para dividir um corpo de texto em partes menores a fim de facilitar a análise que se tenha a intenção de fazer. Esses métodos são: *n-grams* e *tokenization*.

Também foi abordado a noção de categorização de texto como uma ferramente útil em certos cenários, por exemplo, categorização de e-mails (*inbox* ou *spam*) e, dado um texto, esse texto expressa um sentimento negativo ou positivo? Esse último exemplo pode parecer em avaliações de produtos em sites de vendas. Os meodos de classificação abordados foram: *bag-of-words model* e *naive Bayes* com *Lapace smoothing*.
